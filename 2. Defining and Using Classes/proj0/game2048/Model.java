...
public class Model extends Observable {
    ...
    private int Finddestination(Tile t) {
        //Find the final row after moving

        int c = t.col();
        int r = t.row();
        int rd = r;
        //Find row number joint to non-null tile when moving up.
        if (t != null && r < 3) {
            while (rd < 3 && board.tile(c, rd + 1) == null) {
                rd ++;
            }
            //After moving to joint non-null tile.
            //Move up one and merge if they are same value.
            //mr means merge row, mu
            for (int rt = 3; rt > r; rt --) {
                if (board.tile(c, rt) != null) {
                    int rp = rt;
                    int rm = rp - 1;
                    while (board.tile(c, rm) == null) {
                        rm -= 1;
                    }
                    Tile tp = board.tile(c, rp);
                    Tile tm = board.tile(c, rm);
                    tp.merge(c,rp,tm);
                    int v = tp.value();
                    if (v == tm.value()) {
                        rd++;
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        // TODO: Fill in this function. DONE!
        boolean ept = emptySpaceExists(b);
        boolean mte = maxTileExists(b);
        boolean alome = false;
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                Tile t = b.tile(i, j);
                if (ept == true && mte == false) {
                    alome = true;
                } else if (i < 3 && j < 3) {
                    Tile tipj = b.tile(i + 1, j);
                    Tile tijp = b.tile(i, j + 1);
                    if (t.value() == tipj.value() || t.value() == tijp.value()) {
                        alome = true;
                    }
                } else if (i == 3 && j < 3) {
                    Tile tijp = b.tile(i, j+1);
